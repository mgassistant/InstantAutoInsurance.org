...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    // Fail-open: no key configured, don't block real prospects.
    return NextResponse.json({ ok: true, checked: false, reason: "no_api_key" });
  }

  const url =
    `https://api.zerobounce.net/v2/validate?api_key=${encodeURIComponent(apiKey)}` +
    `&email=${encodeURIComponent(email)}` +
    `&ip_address=${encodeURIComponent(getClientIP(req))}`;
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ ok: true, checked: false, reason: `http_${res.status}` });
    }
    const data = (await res.json()) as { status?: string; sub_status?: string };
    const status = data.status || "unknown";
    return NextResponse.json({
      ok: !BLOCK_STATUSES.has(status),
      checked: true,
      status,
